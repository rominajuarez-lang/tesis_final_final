...
def leer_base(uploaded_file):
    ...
    requeridas_maestro = ["product_id", "unit_cost"]
    requeridas_ventas = ["date", "product_id", "demand_real"]
    requeridas_forecast = ["date", "product_id", "forecast_company"]

    for col in requeridas_maestro:
        if col not in maestro.columns:
            raise ValueError(f"Falta la columna {col} en Maestro_SKU")

    for col in requeridas_ventas:
        if col not in ventas.columns:
            raise ValueError(f"Falta la columna {col} en Ventas_Historicas")

    for col in requeridas_forecast:
        if col not in forecast.columns:
            raise ValueError(f"Falta la columna {col} en Forecast_Comercial")

    maestro["product_id"] = maestro["product_id"].astype(str).str.strip()
    ...
    forecast["product_id"] = forecast["product_id"].astype(str).str.strip()
...
# MODELOS
# =========================================================
def pred_regresion(serie, pasos):
    y = np.array(serie, dtype=float)

    if len(y) == 0:
        return np.zeros(pasos)
...
def generar_prediccion(serie, metodo, pasos):
    if metodo == "Regresión lineal":
        return pred_regresion(serie, pasos)
    elif metodo == "SES":
        return pred_ses(serie, pasos)
    elif metodo == "ARIMA":
        return pred_arima(serie, pasos)
    else:
        return pred_ses(serie, pasos)
...
# =========================================================
# EVALUACIÓN 2025 Y PRONÓSTICO 2026
# =========================================================
def evaluar_sku(product_id, ventas, forecast_empresa, maestro):
    ...
    propuestos["Abs Bias"] = propuestos["Bias (%)"].abs()
    mejor = propuestos.sort_values(["wMAPE (%)", "Abs Bias"]).iloc[0]
    ...
    return {
        ...
        "tabla": tabla.drop(columns=["Abs Bias"], errors="ignore"),
...
def evaluar_todos(ventas, forecast_empresa, maestro):
    ...
    return df_resumen, detalles, df_forecast_2026
...
def grafico_modelo(fechas, real, empresa, propuesto, modelo):
    ...
    fig.add_trace(go.Scatter(
        ...
        y=propuesto,
        mode="lines+markers",
        name=f"Forecast propuesto - {modelo}"
    ))
...
def grafico_2026(df_2026_sku):
    fig = px.line(
        ...
        y="forecast_2026",
        markers=True,
        title="Pronóstico mensual 2026 con mejor modelo seleccionado"
    )
...
resumen, detalles, df_forecast_2026 = evaluar_todos(
    ventas,
    forecast_empresa,
    maestro
)
...
if resumen.empty:
    ...
    st.write("Años en ventas:", sorted(ventas["date"].dt.year.dropna().unique()))
    st.write("Años en forecast empresa:", sorted(forecast_empresa["date"].dt.year.dropna().unique()))
    st.write("SKUs en ventas:", ventas["product_id"].nunique())
    st.write("SKUs en forecast empresa:", forecast_empresa["product_id"].nunique())
    st.write("Ejemplo ventas:")
    st.dataframe(ventas.head(20), use_container_width=True)
    st.write("Ejemplo forecast empresa:")
    st.dataframe(forecast_empresa.head(20), use_container_width=True)
    st.stop()
...
with tab1:
    ...
    for metodo in METODOS:
        ...
        with col_tabla:
            tabla_modelo = r["tabla"][
                r["tabla"]["Modelo"].isin(["Forecast empresa", metodo])
            ].copy()
...
with tab2:
    ...
    producto_2026 = st.selectbox(
        "Seleccionar SKU para 2026",
        sorted(df_forecast_2026["product_id"].unique())
    )

    df_sku_2026 = df_forecast_2026[
        df_forecast_2026["product_id"] == producto_2026
    ].copy()

    st.plotly_chart(
        grafico_2026(df_sku_2026),
        use_container_width=True
    )
